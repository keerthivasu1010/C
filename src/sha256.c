...
void sha256_update(SHA256_CTX *ctx, const uint8_t data[], size_t len)
{
    ...
    /* Top up a partially filled internal buffer first so the bulk
     * path below always starts on a 64-byte block boundary. */
    if ((ctx->datalen != 0U) && (len != 0U))
    {
        size_t needed = 64U - ctx->datalen;
    ...
    /* Bulk path: feed whole 64-byte blocks straight from the caller's
     * buffer, avoiding a byte-by-byte copy through ctx->data. */
    ...
    /* Stash any remaining partial block for the next call/final(). */
    if (offset < len)
    {
        size_t remaining = len - offset;
        memcpy(&ctx->data[ctx->datalen], &data[offset], remaining);
        ctx->datalen += (uint32_t)remaining;
    }
}
...
int sha256_hash_buffer(const uint8_t *input, size_t len, char outHex[SHA256_HEX_BUF])
{
    ...
    /* Table-driven hex encode: avoids a format-string parse per byte. */
    for (i = 0U; i < SHA256_BLOCK_SIZE; ++i)
    {
        outHex[i * 2U]       = HEX_DIGITS[(digest[i] >> 4U) & 0x0fU];
        outHex[(i * 2U) + 1U] = HEX_DIGITS[digest[i] & 0x0fU];
    }
